...
public extension Timer {
    ...
    class func fg_scheduledTimer(interval:TimeInterval,repeats:Bool, block: ((Timer) -> ())?) -> Timer {
    ...
    @objc private class func fg_timerAction(_ sender:Timer){
        let block:((_ t:Timer)->Void)?=objc_getAssociatedObject(self, &TimerBlockActionKey) as? ((Timer) -> ())
